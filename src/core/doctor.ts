...
async function checkState(home: string, out: DoctorFinding[]): Promise<void> {
  ...
  if (!existsSync(auditFile)) {
    ...
    out.push({
      ...
      message:
        corrupt > 0
          ? `audit log: ${corrupt}/${total + corrupt} corrupt lines (those changes can't be rolled back)`
  ...
  if (existsSync(lockPath)) {
    ...
    out.push({
      ...
      message:
        age > STALE_LOCK_MS
          ? `lock: ${lockPath} is ${Math.round(age / 60000)} min old — likely stale; remove it if no fleet operation is running`
          : `lock: an operation appears to be in progress (${lockPath})`,
    });
  }

  const delegated = join(home, 'delegated.jsonl');
  if (existsSync(delegated)) {
    ...
    out.push({
      ...
      message:
        corrupt > 0
          ? `delegated ledger: ${corrupt}/${total + corrupt} corrupt lines`
...
export async function runDoctor(
  ...
  if (!config) {
    try {
      ...
      findings.push({ category: 'config', level: 'error', message: `config.json does not load: ${msg(e)}` });
    }
  }
