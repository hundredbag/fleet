...
const V1_CSS = `
  ...
  * { box-sizing:border-box; }
  ...
  .vsw a { color:var(--text-3); text-decoration:none; padding:2px 6px; border-radius:5px; }
  .vsw a:hover { color:var(--text); }
  .vsw a.active { color:var(--accent); font-weight:700; }
`;
...
const V2_CSS = `
  ...
  * { box-sizing:border-box; }
  ...
  .vsw a { color:var(--text-3); text-decoration:none; padding:2px 6px; border-radius:5px; font-weight:600; }
  .vsw a:hover { color:var(--text); background:var(--hover); }
  .vsw a.active { color:var(--accent); font-weight:700; }
`;

export function renderPage(variant?: string): string {
  const V2 = variant === 'v2';
  return `<!doctype html>
<html lang="ko" data-theme="dark" data-v="${V2 ? '2' : '1'}">
<head>
<meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<meta name="referrer" content="no-referrer">
<title>fleet</title>
<style>${V2 ? V2_CSS : V1_CSS}</style>
</head>
<body>
<header>
  <div class="brand">
...
<div class="foot"><span data-t="foot"></span> <span class="vsw"><a href="?v=1" data-v="1">v1</a> · <a href="?v=2" data-v="2">v2</a></span></div>
...
// version switch: carry the session token (the URL has none after strip) and
// mark the active variant
(function(){
  const active = document.documentElement.getAttribute('data-v');
  document.querySelectorAll('.vsw a').forEach(function(a){
    if(a.getAttribute('data-v') === active) a.className = 'active';
    a.addEventListener('click', function(e){
      e.preventDefault();
      location.href = location.pathname + '?v=' + a.getAttribute('data-v') +
